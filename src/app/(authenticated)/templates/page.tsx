...
export default function TemplatesPage() {
  ...
  // --- REACT QUERY HOOKS ---
  const { data: levels = {data:[]} } = useQuery({ queryKey: ["levels"], queryFn: () => fetchLevels() });
  const { data: topics = [] } = useQuery({ queryKey: ["topics"], queryFn: () => fetchTopics({}) });
  
  const { data: termsData = [] } = useQuery({ 
    queryKey: ["schedule_terms"], 
    queryFn: fetchTerms 
  });
  ...
  const handleConfirmPublish = async () => {
    ...
    try {
      ...
      toast.success(`Template ${actionWord}ed successfully`);
    } catch (error) {
      console.error(error);
      toast.error(`Failed to ${actionWord} template`);
    } finally {
      // Cleanup
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-1">
            <TooltipProvider>
                <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setDeleteTarget(item)}><Trash2 className="h-4 w-4 text-muted-foreground hover:text-destructive" /></Button></TooltipTrigger><TooltipContent>Delete</TooltipContent></Tooltip>
            </TooltipProvider>
            <TooltipProvider>
                <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleEdit(item)}><Pencil className="h-4 w-4 text-muted-foreground hover:text-primary" /></Button></TooltipTrigger><TooltipContent>Edit</TooltipContent></Tooltip>
            </TooltipProvider>
            <TooltipProvider>
                <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditingItem(item); setPreviewOpen(true); }}><Eye className="h-4 w-4 text-muted-foreground hover:text-blue-500" /></Button></TooltipTrigger><TooltipContent>Preview</TooltipContent></Tooltip>
            </TooltipProvider>
    ...
    <div className="space-y-4 p-4 md:p-8">
      <Card className="relative overflow-hidden">
          ...
          <div className="flex items-center space-x-2 w-full md:w-auto">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button className="gap-2"><Plus className="h-4 w-4" /> New Item <ChevronDown className="h-4 w-4" /></Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => handleCreate("normal")}>New E-lesson</DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleCreate("topical")}>New Topical</DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleCreate("revision")}>New Active Revision Class</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </CardHeader>
        ...
        <CardContent>
          {isInitialLoading ? (
            ...
            <>
                <CTable columns={columns} data={filteredData} state={{ rowSelection }} onRowSelectionChange={setRowSelection} />
      ...
      {dialogOpen && (
        <LessonTemplateDialog 
          ...
          topics={topics} 
          terms={termsData} 
          onStep1Submit={async () => { loadData(); }}
        />
      )}

      {/* DELETE CONFIRMATION */}
      <DeleteTemplateDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)} onConfirm={handleDeleteConfirm} />
