...
import { TEMPLATE_DETAIL_QUERY_KEY } from "@/const/queryKey";
...
export function PreviewLessonDialog({
  ...
  const { data: details, isLoading } = useQuery({
    queryKey: [TEMPLATE_DETAIL_QUERY_KEY, item?.id],
    queryFn: () => fetchLessonDetail(item!.id),
    enabled: open && !!item?.id,
  });

  const isTopical = details?.topical === 1 || item?.topical_label === "Yes";
  const isRevision = details?.active_revision === 1 || item?.active_revision_label === "Yes";
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-5xl! h-[90vh] flex flex-col p-0 gap-0" onInteractOutside={e => e.preventDefault()} showCloseButton={false}>

          <DialogHeader className="px-6 py-4 border-b shadow-sm z-10 flex flex-row justify-between items-center space-y-0">
              ...
              <Tabs defaultValue={showReviewTab ? "review" : "main"} className="flex h-full">
                ...
                <div className="w-48 border-r pt-4">
                  <TabsList className="flex flex-col h-auto w-full bg-transparent gap-1 p-2">
                    {showReviewTab && (
                      <TabsTrigger value="review" className="w-full justify-start gap-2 data-[state=active]:shadow-sm border border-transparent data-[state=active]:border-slate-200">
                        <ClipboardList className="h-4 w-4" /> Review
                      </TabsTrigger>
                    )}
                    <TabsTrigger value="main" className="w-full justify-start gap-2 data-[state=active]:shadow-sm border border-transparent data-[state=active]:border-slate-200">
