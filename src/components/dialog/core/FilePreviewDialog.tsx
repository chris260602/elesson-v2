...
export function FilePreviewDialog({ open, onOpenChange, file }: FilePreviewDialogProps) {
  ...
  const isImage = !isPdf && (file?.type === 'image' || file?.type === 'graphics' || ['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(ext));
  const isVideo = !isPdf && !isImage && (file?.type === 'video' || file?.type === 'videos' || ['mp4', 'mov', 'avi', 'webm'].includes(ext));
  const isOffice = !isPdf && !isImage && !isVideo && (['doc', 'docx', 'ppt', 'pptx', 'xls', 'xlsx'].includes(ext) || file?.type === 'office');
  useEffect(() => {
    let isMounted = true;
