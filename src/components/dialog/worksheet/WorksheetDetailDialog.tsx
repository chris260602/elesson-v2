...
import { Loader2, FileText, Video, Image as ImageIcon, QrCode } from "lucide-react"; // Import Loader2
...
export function WorksheetDetailDialog({ 
  ...
  const FileRow = ({ file, icon: Icon, label }: { file?: WorksheetFile, icon: any, label: string }) => {
    if (!file) return null;
    return (
      <div className="flex items-start gap-4 p-3 border rounded-lg mb-2 bg-slate-50 hover:bg-slate-100 transition-colors">
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl! max-h-[90vh] overflow-y-auto min-h-[500px] flex flex-col">
        <DialogHeader>
            ...
            <div className="flex-1">
                ...
                <div className="space-y-4 animate-in fade-in duration-300">
                    ...
                    <div className="flex justify-end pt-8 gap-2">
                        ...
                        <Button onClick={() => setStep(2)}>Next: Files</Button>
                    </div>
                </div>
                )}
