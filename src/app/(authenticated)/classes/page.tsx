...
const TagCell = ({ item }: { item: ClassType }) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      toast.success(`Tag updated to ${tagName}`);
...
export default function ClassesPage() {
  ...
  // Queries
  const { data: classes = [], isLoading, isRefetching, refetch } = useQuery({
    queryKey: ["classes"],
  ...
  const columns: ColumnDef<ClassType>[] = [
    {
      ...
      header: "Class Name",
      cell: ({ row }) => <span className="font-semibold text-slate-700">{row.getValue("class_id")}</span>
    },
    {
      accessorKey: "tags",
      header: "Class Tag",
      ...
      <Card className="relative overflow-hidden border-slate-200 shadow-sm">
        ...
        <CardHeader className="space-y-6 pb-6">
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="flex items-center gap-2">
                    <CardTitle className="text-xl font-bold text-slate-800">Classes Management</CardTitle>
                    <TooltipProvider>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                ...
                                >
                                    <RotateCcw className={`h-4 w-4 ${isLoading || isRefetching ? 'animate-spin' : ''}`} />
                                </Button>
                            </TooltipTrigger>
                ...
                <div className="space-y-2 md:col-span-1">
                    ...
                    >
                        <SelectTrigger className="bg-white w-full border-slate-200">
                           {renderSelectContent(isLoading, "All Tags")}
                        </SelectTrigger>
                        <SelectContent>
