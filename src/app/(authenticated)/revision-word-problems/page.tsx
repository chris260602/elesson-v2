...
import { LEVELS_QUERY_KEY, TOPICS_QUERY_KEY, REVISION_QUERY_KEY } from "@/const/queryKey";
...
import { fetchQuestions, fetchQuestionDetails, deleteQuestion } from "@/apiRoutes/revision-words";
...
export default function RevisionPage() {
    ...
    const { data: topicsResponse = [] } = useQuery({
        ...
        queryFn: () => {
            ...
            return fetchTopics({ level: param });
        },
    });

    const topicOptions = topicsResponse;
    ...
    const {
        ...
        queryKey: [REVISION_QUERY_KEY, filters.level, filters.topic],
        queryFn: () => {
    ...
    const handlePreview = async (item: RevisionProblem) => {
        ...
        try {
            ...
            showErrorMessage("Failed to fetch preview");
    ...
    return (
        <div>
            ...
            <SwapDialog
                ...
                onSuccess={(newNumber: string | number) => {
                    // Optimistic Cache Update
                    const queryKey = [REVISION_QUERY_KEY, filters.level, filters.topic];
                    const num = Number(newNumber);
                    const oldNum = Number(editingItem.question_no);

                    queryClient.setQueryData(queryKey, (old: any) => {
                        if (!old) return old;
                        // Map through items and swap numbers
                        const updatedData = (old.data || old).map((q: RevisionProblem) => {
                            if (q.id === editingItem.id) return { ...q, question_no: num };
                            // If we find the item that currently has the target number, swap it back (optional but feels better)
                            if (Number(q.question_no) === num) return { ...q, question_no: oldNum };
                            return q;
                        });
                        return Array.isArray(old) ? updatedData : { ...old, data: updatedData };
                    });

                    loadData(); // Background refetch
                    setIsSwapOpen(false);
                }}
            />
