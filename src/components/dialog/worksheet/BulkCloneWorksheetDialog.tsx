...
export function BulkCloneWorksheetDialog({ open, onOpenChange, years }: BulkCloneWorksheetDialogProps) {
  ...
  useEffect(() => {
    const fetchLevels = async () => {
        ...
        try {
            ...
            toast.error("Could not load levels for this year.");
            setAvailableLevels([]);
        } finally {
  ...
  const executeClone = async () => {
    ...
    try {
        ...
        toast.success("Mass Clone Worksheets Successfully");
        ...
        console.error(error);
        toast.error(error.response?.data?.message || "Failed to clone worksheets");
    } finally {
        setIsCloning(false);
    }
  };
